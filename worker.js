	...
	try {
		...
		if (target === "management") {
			const mockExtensionInfo = {
				...
				enabled: true,
				homepageUrl:
					"https://chromewebstore.google.com/detail/deojfdehldjjfmcjcfaojgaibalafifc",
...
async function checkForUpdate() {
	try {
		const response = await fetch(
			"https://api.github.com/repos/rishit537/NeoPass/releases/latest",
		);
		const data = await response.json();
		...
		if (compareVersions(latestVersion, currentVersion) > 0) {
			...
			const { lastUpdateDismissed } = await chrome.storage.local.get([
				"lastUpdateDismissed",
			]);
			...
			if (
				!lastUpdateDismissed ||
				currentTime - lastUpdateDismissed > showNotificationTimeout
			) {
						...
						func: function (msg, version) {
							...
							gradientContainer.onclick = (e) => {
								if (
									...
									window.open(
										"https://github.com/rishit537/NeoPass/releases/latest",
									);
								}
							};
				...
				chrome.scripting.executeScript(
					...
					(results) => {
						if (results && results[0] && !results[0].result) {
							showToast(
								tab.id,
								"Paste operation failed. Please try again.",
								true,
							);
						}
					},
				...
				chrome.scripting.executeScript(
					...
					(results) => {
						if (results && results[0] && !results[0].result) {
							showToast(
								tab.id,
								"Paste by typing operation failed. Please try again.",
								true,
							);
						}
					},
	...
	if (selectedText) {
		...
		if (bestAnswers) {
			if (Array.isArray(bestAnswers) && bestAnswers.length > 0) {
				...
				showNPTELToast(
					tabId,
					"Answer not found.\nPlease select only the question.",
					true,
				);
			}
		} else {
		...
		switch (errorType) {
			case "rateLimit":
				showToast(
					...
					true,
					detailedInfo ||
						"You have exceeded your request limit. Please wait before trying again.",
				);
				break;
			case "auth":
				showToast(
					...
					true,
					detailedInfo ||
						"Please log in or refresh your session to continue using the service.",
				);
				break;
			case "forbidden":
				showToast(
					...
					true,
					detailedInfo ||
						"Access to this feature is restricted. Please check your account status.",
				);
				break;
			case "server":
				showToast(
			...
			default:
				showToast(
					...
					true,
					detailedInfo ||
						"An unexpected error occurred. Please try again after 30 seconds.",
				);
		}
	} else {
			...
			if (isTyped) {
				...
				chrome.scripting
					.executeScript({
						...
						func: function (code) {
							console.log(
								"[INJECTED] Calling _neopassStartTyping, code length:",
								code.length,
							);
							if (typeof window._neopassStartTyping === "function") {
								...
								console.error(
									"[INJECTED] _neopassStartTyping not found on window",
								);
							}
						},
						args: [cleanedCode],
...
async function queryRequest(
	...
	if (!canMakeRequest()) {
		console.log(
			"[Request Block] Request blocked - another request is in progress",
		);
		return {
	...
	try {
		...
		if (customAPIConfig.useCustomAPI && customAPIConfig.apiKey) {
			const result = await queryCustomAPI(
				text,
				isMCQ,
				isMultipleChoice,
				customAPIConfig,
		...
		console.log(
			"[queryRequest] Sending request to API",
			API_URL,
			"with body:",
			body,
		);
					...
					if (response.status === 429) {
						...
						if (
							...
							if (errorData.details) {
								...
								detailedInfo =
									"You have reached your token limit for this billing period.";
							}
						} else if (
							errorData.message &&
							...
							errorMessage =
								"Please wait for your previous request to complete.";
							detailedInfo =
								"Multiple simultaneous requests are not allowed. Please wait a moment before trying again.";
						} else {
							errorMessage =
						...
						if (
							...
							errorData.message &&
							errorData.message.includes("star")
						) {
						...
						detailedInfo =
							errorData.error || `HTTP ${response.status}: ${errorMessage}`;
					}
				} catch (parseError) {
...
async function queryCustomAPI(text, isMCQ, isMultipleChoice, config) {
	...
	try {
			...
			case "custom":
				if (!customEndpoint) {
					return {
						...
						errorType: "config",
						detailedInfo:
							"Please configure a custom API endpoint in the extension settings.",
					};
				}
		...
		return {
			...
			detailedInfo:
				error.message ||
				"Failed to connect to the custom AI API. Please check your configuration.",
		};
	}
}
	...
	try {
			...
			try {
				...
				if (response.status === 429) {
					...
					if (
						...
						errorMessage =
							"Please star the repository to use the chat feature.";
					} else {
						errorMessage =
					...
					errorMessage =
						"Your message couldn't be processed. Try rephrasing or shortening it.";
				} else {
					errorMessage =
						errorData.message ||
		...
		if (response.ok && data.success) {
			...
			if (data.newAccessToken) {
				...
				console.log(
					"✅ Access token auto-refreshed during chat request and stored",
				);
			}
		...
		function (tabs) {
			if (tabs[0]) {
				showOpacityLevelToast(
					tabs[0].id,
					`Toast opacity set to: ${currentOpacityLevel}`,
				);
			}
		},
	);
